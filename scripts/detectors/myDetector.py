...
class DetectorParams:
    ...
    def __init__(self, verbose=True):
        ...
        self.use_tf = rospy.get_param("use_tf", True) # setting default value to True

        # Path to the trained conv net
        model_path = rospy.get_param("~model_path", "../../tfmodels/ssd_mobilenet_v1_coco.pb") # updated to coco
...
class Detector:
    ...
    def __init__(self):
        ...
        if self.params.use_tf:
            ...
            with self.detection_graph.as_default():
                ...
                config = tf.ConfigProto() # NEW, from detector_mobilenet
                config.gpu_options.allow_growth = True # NEW, from detector_mobilenet
            self.sess = tf.Session(graph=self.detection_graph, config=config) # Adding config kwarg

        # camera and laser parameters that get updated. 
        # NOTE from dan: these appear to just be an initialization? seem to get updated in other functions based on callbacks
        self.cx = 0.
        self.cy = 0.
        self.fx = 1.
        self.fy = 1.
        ...
        rospy.Subscriber('/camera/image_raw', Image, self.camera_callback, queue_size=1, buff_size=2**24) # Adding buff_size kwarg
        rospy.Subscriber('/camera/image_raw/compressed', CompressedImage, self.compressed_camera_callback, queue_size=1, buff_size=2 ** 24) # Added
    ...
    def run_detection(self, img):
        ...
        if self.params.use_tf:
            ...
            with self.detection_graph.as_default():
                (boxes, scores, classes, num) = self.sess.run([self.d_boxes,self.d_scores,self.d_classes,self.num_d], feed_dict={self.image_tensor: image_np_expanded})

            return self.filter(boxes[0], scores[0], classes[0], int(num[0])) # Adding int() to num to fix float error
    ...
    def project_pixel_to_ray(self, u, v):
        ...
        ########## Code starts here ##########
        # NOTE: Added code from mobilenet
        x = (u - self.cx) / self.fx
        y = (v - self.cy) / self.fy
    ...
    def camera_info_callback(self, msg):
        ...
        ########## Code starts here ##########
        # NOTE: added info from mobilenet
        self.cx = msg.P[2]
        self.cy = msg.P[6]
        self.fx = msg.P[0]
        self.fy = msg.P[5]
